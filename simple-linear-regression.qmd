...
Suppose we wanted to know the potential timber yield from a stand of trees. The direct but destructive way to find ot would be to chop down every tree and weigh it, but a more useful route would be if we could estimate it from variables that we could easily measure, for example the girth of the trunk as measured by its diameter at breast height, and its height, which an easily be found using a protractor, a tape measure and simple trigonometry.
...
Alternatively, and more simple perhaps, use the project foler you downloaded at the beginning of this chapter. That contains a working notebook - the one on plant growth, that uses the plant growth csv file in the data subfoled. You could use this for your purposes - all you need to do is to put *your* data as a csv file into the data subolder, and then change file, data frame and column names in the notebook as appropriate.
...
**Report the conclusions in plain English**