...
```{r}
# how man individuals from each species?
n_replicates <- 10
```
...
penguins <- penguins_raw |>
  ...
  group_by(species) |>
  slice_sample(n = n_replicates) |>
  ungroup() |>
  mutate(ID=1:(3*n_replicates))
...
penguins <- mutate(penguins,pmeans = c(rep(p_means$pmean[1],n_replicates),
                            rep(p_means$pmean[2],n_replicates),
                            rep(p_means$pmean[3],n_replicates)))
...
SST is a measure of the total variability of the data set. For our penguins we find that SST = `r round(SST,3)`
...
```{r}
# calculate quantities for degrees of freedom
...
GMS <- SSG / (k - 1) # group (species, in this case) mean squares
EMS <- SSE / (n - k) # error mean squares
TMS <- SST / (n - 1) # total mean squares
```
...
The shape of the F distribution depends on the degrees of freedom of GMS and EMS, and we normally specify it by giving the values of each. In @fig-f-pdfs below we show F distributions for 2 and 27 degrees of freedom (ie 3 species, so *k* = 3, so the degrees of freedom of GMS = *k*-1 = 2, and 10 individuals per species, so *n*  = 3 x 10 = 30, and hence the degrees of freedom of EMS = *n*-*k* = 30 - 3 = 27), and for 10 and 27 degrees of freedom.

```{r}
#| label: fig-f-pdfs
#| fig-cap: "The F-distributions for (left) 2 and 27 degrees of freedom and (right) 10 and 27 degrees of freedom"
...
We find for our penguin data that the F ratio is `r round(F_penguins,1)`

If we look at the *F* distribution in the left-hand figure in @fig-f-pdfs the one that corresponds to our case, with 2 and 27 degrees of freedom, we see that it has already fallen to near zero by the time *F* is equal to 5 or 6. There is essentially zero area beneath the curve to the right of the much larger F value we found for our data, *F* = `r round(F_penguins,1)`. That means there is essentially zero chance of getting an *F* value this big or bigger if the null hypothesis were true (ie the p-value is esentially zero), and so we can confidently reject the null hypothesis.

## What does the ANOVA result tell us?

The null hypothesis of the ANOVA is that all the samples are drawn from populationswit the same mean. If the F-value is so large that we reject this null hypothesis, then we infer that at least two of the population means differ. We not learn from the ANOVA where the difference or differences lie.
...
penguin.model <- lm(bill_shape ~ species, data = penguins)
anova(penguin.model)
```
