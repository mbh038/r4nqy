...
This now allows us to plot the deviations of each individual from this grand mean and to calculate what we will call **SST: the total sum of squares**. This is the sum of the squared differences between each individual penguin beak shape and the grand mean. In this and other measures of variability we square the differences so that we do not  distinguish between positive and negative deviations.
...
**Species Mean Square** (SMS) = SSG / *k* - 1. This is the variation per *df* between individuals of different species.
...
We call this ratio the __*F*-ratio__. This is the so-called 'test statistic that ANOVA calculates from your data.  *F*-ratios can never be negative since they are the ratio of two mean square values, both of which must be non-negative, but there is no limit to how large they can be.
