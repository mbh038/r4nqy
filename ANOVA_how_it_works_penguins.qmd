...
This now allows us to plot the deviaions of each individual from this grand mean
...
p_means<-penguins |>
  group_by(species) |>
  summarise(n = n(), pmean = mean(bill_shape))
p_means
...
g4<-ggplot()+
  ...
  scale_y_continuous(limits=c(1,4))+
  labs(x='ID',y='Bill shape ',title="SSG: group sum of squares")+
  theme_cowplot()
```
...
First, we note that SSG and SSE partition between them the total variability in the data:
