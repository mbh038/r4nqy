...
The notable statisticians Karl Pearson and Alice Lee carried out a study to investigate the resemblance between children and their parents. As part of the study, they measured the heights of 1078 parents and of their children at maturity. Here we use a subset of of their data, that comprises 746 parent-child pairs, available in the `HistData` package. The heights of the children are plotted against the heights of the parents in the plots below, where we distinguish between father-son and mother-daughter pairs.
...
Here we use data from [Our World in Data: Human Height](https://ourworldindata.org/human-height#all-charts_)

The figure below shows the mean heights of adult men vs adult women in many different countries who were born in 1996. There is a strong positive and linear correlation between these heights (r~sp~ = 0.941, p \< 0.001). The Spearman's rank correlation coefficient is calculated because the male mean heights are not normally distributed among themselves.

```{r}
adult_heights_filepath<-here("data",
                     "mean-height-males-vs-females",
                     "mean-height-males-vs-females.csv")
adult_heights_1996 <- read_csv(adult_heights_filepath) |>
  clean_names() |>
  filter(year == 1996) |>
  drop_na()
```
...
```{r}
# shapiro.test(adult_heights_1996$mean_male_height_cm)
# shapiro.test(adult_heights_1996$mean_female_height_cm)
# cor.test(adult_heights_1996$mean_male_height_cm,adult_heights_1996$mean_female_height_cm, method="spearman")$p.value
```

```{r}
adult_heights_1996 |>
  ggplot(aes(x = mean_male_height_cm,
             y = mean_female_height_cm, colour = world_region_according_to_owid)) +
             geom_point() +
             labs(x = "Mean female height (cm)",
                  y = "Mean male height (cm)",
                  colour = "Region") +
  annotate("text", x = 179, y = 151, label = deparse(bquote(italic(r)[sp]==0.941~","~italic(p)<0.001)), parse = TRUE) +
...
pressure_wind_speed |>
  ...
  geom_jitter(width = 2, height = 2, alpha = 0.5, colour = "#386cb0") +
...
swh_vs_cyclone_size |>
  ...
  geom_point(size = 3, alpha = 0.5, colour = "#386cb0") +
...
In a [-@jovan2008] paper, Jovan *et al*. investigated the utility of using lichen as bioindicators of air quality. Is there, they asked, an association between air quality and the abundance of this or that species of lichen?
...
Here we look at figures published in a paper by Lauber and co-workers [-@lauber2009] in which they investigate the evidence for an association between soil pH and the abundance of various species of bacteria.
...
hr_le  |>
  ...
  geom_point(size = 3, colour="gray60", colour = "#386cb0") +
