...
If we have abundance data for each species at a site, for example percentage cover, we have more information than if we simply had presence-absence data. Hence, not surprisingly, we can do more with it. Given the proportion $p_i$ of each of several species $i$ within a population, we can estimate the so-called diversity of the population in one of several ways from the proportions $\hat p_i$ of each species within a sample. The proportions can be of total counts or of total percentage cover.
...
Note how this measure $1-D$ makes sense as a diversity index. If a site had one species that comprised 100% of the cover of the site, ie $p = 1$ for that species and $p = 0$ for every other species, then the diversity of that site would be low, right? The value of $D$ would be 1 and the Simpson's index value $1-D$ would thus be zero.
