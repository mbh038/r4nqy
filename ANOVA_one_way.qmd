...
## Example: Female penguin data from the palmerpenguins package
...
In this chapter we are only interested in the female penguins, so we narrow the data set down to just those, using the `filter()` function and save the results in a data frame called `females`:

```{r}
females <- penguins_clean |>
  filter(sex == "female")
glimpse(females)
```
...
Looking at this table, does it look as though the species weigh the same?
...
What do you think now about body mass differences between the species?
...
To prepare the penguin data for our purposes here, narrow the full data set down to just the females, using the `filter()` function, and then make sure there are no lines with missing values, using `drop_na()`. We save this cleaned data set in an object called `females`.
...
Now, we summarise these values to find the number of individuals, the mean body mass for each species, and the standard errors of those means:
...
By now you probably have a good idea what the answer is as to our question but now we will move on to the actual statistics test, in this case a one-way ANOVA.
...
* Do post-hoc pairwise comparisons using `emmeans()` in the manner shown in @post-hoc
