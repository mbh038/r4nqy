...
In this example the data are grouped by species, the sample size is found then the mean value of all the columns that contain numerical data are calculated, not just an overall value for the whole column, but for each species

```{r}
summary_by_species <- penguins |>
  group_by(species) |>
  summarize(n = n(),
