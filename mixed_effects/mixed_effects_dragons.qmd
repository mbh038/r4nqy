...
qq plot - shpould get a straight line:
```{r}
...
dragons |>
  ...
  labs(x = "Mountain Range",
       y = "Test Score")
```
or we could plot a scatter plot:
...
From the above plots it looks like thee is a correlation within mountain ranges 0f both dragon body length and test scores. This confirms that our observations from within each of the ranges aren't independent. We can't ignore that.So what do we do?
...
This presents problems - not only we are **hugely decreasing our sample size**, but we are also **increasing the type one error rate due to carrying out multiple comparisons**. Not ideal!

## Modify the current model

## We want to use all the data, but account for the data coming from different mountain ranges. Let's add mountain range as a fixed effect to our basic.lm
...
dragons <- dragons |>
  mutate(sample = factor(mountainRange:site))
```
